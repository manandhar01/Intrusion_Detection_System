        ...
        fs.readFile(filename, "utf-8", (err, data) => {
            if (err) {
                ...
                if (prevData !== data) {
                    ...
                    let formattedData = newData.split("\n");
                    formattedData = /\d/.test(formattedData[0])
                        ? formattedData.slice(0, -1)
                        : formattedData.slice(1, -1);
                    const removeIndices = [0, 1, 2, 4, 5].sort((a, b) => b - a);
                    formattedData.forEach((fd) => {
                        fd = fd.slice(0, -1).split(",");
                        for (let i = 0; i < removeIndices.length; i++) {
                            fd.splice(removeIndices[i], 1);
                        }
                        let newfd = Array(77).fill(0);
                        capturedArrayFormat.forEach((capture, index) => {
                            newfd[requiredArrayFormat.indexOf(dict[capture])] =
                                fd[index];
                        });
                        formattedDataList.push(newfd);
                    });
